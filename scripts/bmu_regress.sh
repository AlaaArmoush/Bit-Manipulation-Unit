...
for test_name in "${tests[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    if [[ ! -f $run_log ]]; then
      ...
      pass_verdict_count=$(
        grep -c '\[BMU_TEST_PASS\]' "$run_log" || true
      )
      fail_verdict_count=$(
        grep -c '\[BMU_TEST_FAIL\]' "$run_log" || true
      )

      if ((pass_verdict_count == 1 && fail_verdict_count == 0)); then
        :
      elif ((pass_verdict_count == 0 && fail_verdict_count == 1)); then
        append_reason "BMU_TEST_FAIL"
      elif ((pass_verdict_count == 0 && fail_verdict_count == 0)); then
      ...
      if grep -Eq '^(xrun|xmsim|xmelab|xmvlog): \*[FE],' "$run_log"; then
    ...
    if [[ -d $coverage_dir ]]; then
      coverage_file=$(
        find "$coverage_dir" -type f -name '*.ucd' -print -quit 2>/dev/null
      )
    fi
