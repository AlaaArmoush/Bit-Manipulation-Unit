class bmu_csr_read_sequence extends bmu_base_sequence;
  ...
  virtual task body();
    ...
    for (int unsigned i = 0; i < RANDOM_TRIAL_COUNT; i++) begin
      ...
      if (!request.randomize() with {
        ...
        `uvm_fatal(
            "CSR_READ_RANDOMIZE",
            $sformatf("Failed to randomize CSR read trial %0d", i)
        )
      end
      ...
      `uvm_info(
          "CSR_READ_STIMULUS",
          $sformatf(
              "CSR-R-04 random trial %0d: csr_rddata_in=0x%08h",
              i,
              request.csr_rddata_in
          ),
          UVM_MEDIUM
      )
    ...
    `uvm_info(
        "CSR_READ_STIMULUS",
        "CSR-R-05 invalid conflict: CSR read combined with in-scope OR control",
        UVM_MEDIUM
    )
    send_bmu_request(request);
  endtask : body
  ...
  protected task send_legal_csr_read(string request_name, logic [31:0] csr_data);
    ...
    `uvm_info(
        "CSR_READ_STIMULUS",
        $sformatf(
            "Sending legal CSR read '%s': csr_rddata_in=0x%08h",
            request_name,
            csr_data
        ),
        UVM_MEDIUM
